...
`import` reads whole lists, `update` takes only messages numbered above the last one stored and then embeds pending messages, which is what Heroku Scheduler runs daily. `embed` covers every list but ruby-talk by default and skips Redmine notifications.
